...
- **Loan Officer** — handles the leads assigned to them (`loan_officer_id` on the lead); does not create leads. Sees `/leads/list` scoped to their own assigned leads and can mark them billable/non-billable. May belong to a team, which does not widen their own visibility.
- **Team Lead** — scoped to the team they lead (`team_id` on their user document); a team can have several team leads with identical scope. A team holds both agents and loan officers. Sees a team dashboard with per-member lead analytics and campaign breakdowns ([teams/backend/get-team-dashboard](teams/backend/get-team-dashboard)) — agents measured by leads created, loan officers by leads assigned — plus the leads from both. Can view/manage the agents and loan officers on their team ([users/backend/manage-users](users/backend/manage-users)) and activate/deactivate (not delete or promote) their accounts. Cannot change roles or team assignment.
