...
function statusPill(status: ListedLead['status']) {
...
export function LeadCard({ lead }: LeadCardProps) {
	return (
		<Card className="relative rounded-[19px] border-none p-6 shadow-[0px_4px_4px_-3px_rgba(0,0,0,0.25)]">
			<div className="mb-4 flex items-start justify-between">
				...
				<span
					className={`rounded-full px-3 py-1 text-[10px] lg:text-[12px] ${statusPill(
						lead.status,
					)}`}
				>
					{lead.status
